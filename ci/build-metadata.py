...
The result is printed as a lava-test-plans variables file, under EXTRA_METADATA.
"""
import json, re, sys
...
def describe(kernel, recipe, mine):
    ...
    # Keying off that rather than "kernel-image-<type>" keeps this independent
    # of KERNEL_IMAGETYPE, which differs per machine (Image, Image.gz, zImage).
    release = ""
    for p in mine:
...
print(json.dumps({"EXTRA_METADATA": best} if best else {}, indent=2))
