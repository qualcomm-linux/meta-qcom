...
do_compile() {
    ...
    # Overlay the cert-bearing config ELF deployed by firmware-qcom-oem-cert
    # as the machine-selected variant (QCOM_XBL_CONFIG) and as the plain
    # xbl_config.elf the FV entry references, so the capsule firmware volume
    # carries the same image the device boots.  Optional: hamoa has no
    # xbl_config.elf.
    if [ -f "${DEPLOY_DIR_IMAGE}/xbl_config-with-oem-cert.elf" ]; then
        ...
        if [ "${QCOM_XBL_CONFIG}" != "xbl_config.elf" ]; then
            install -m 0644 "${DEPLOY_DIR_IMAGE}/xbl_config-with-oem-cert.elf" \
