...
SRC_URI = "https://github.com/abseil/abseil-cpp/releases/download/${PV}/abseil-cpp-${PV}.tar.gz \
...
# Provide only libraries to fulfill dependencies
do_install:append() {
    rm -fr ${D}${includedir}
    rm -f  ${D}${libdir}/*.so
    rm -fr ${D}${libdir}/cmake
    rm -fr ${D}${libdir}/pkgconfig
}

PACKAGES_DYNAMIC += "^libabsl-legacy-*"
...
python split_dynamic_packages() {
    ...
    libpackages = do_split_packages(
        ...
        output_pattern='libabsl-legacy-%s',
        description="abseil (legacy) shared library %s",
        prepend=True,
        extra_depends='',
    )
    if libpackages:
